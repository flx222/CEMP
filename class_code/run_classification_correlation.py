...
def do_val_models(save_checkpoint_path,ds_val):
    ...
    os.system("cp "+best_ckpt+" "+os.path.join(save_checkpoint_path,"Best_Model_Num_"+best_ckpt.split("_")[-2].split("-")[-1]+".ckpt"))
...
def do_train(dataset=None,ds_val=None, network=None, load_checkpoint_path="", save_checkpoint_path="", epoch_num=1):
    """ do train """
...
def do_eval(dataset=None, network=None, num_class=None, load_checkpoint_path="",do_plot=False,do_train=False):
    ...
    if num_class==2:
        print_result["AUPRC"] = metrics.average_precision_score(true_labels, pred_labels)
...
def run_classifier():
    ...
    if args_opt.do_train==True:
        ...
        do_train(train_data_file_path,ds_val, netwithloss, args_opt.load_checkpoint_url, args_opt.output_url, epoch_num)
