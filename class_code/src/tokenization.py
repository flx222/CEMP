...
class FullTokenizer:
    """
    Full tokenizer for sequences and SMILES strings.
    """
    ...
    def tokenize(self, text, is_smile=False):
        ...
        Tokenize input text.
        Args:
            text: str or list of characters.
        ...
        # Handle input being a list of characters (e.g., from generate_predict_seq)
        ...
        if is_smile:
            # SMILES tokenization: process each character individually with SM_ prefix
            for char in text:
                token = f'SM_{char}'
                if token in self.smile_vocab_dict:
                    tokens_ret.append(token)
                else:
                    tokens_ret.append('[UNK]')  # If not found in SMILES vocab, mark as unknown
        else:
            # Regular tokenization (for sequences like 'seq')
            for token in text:
                # if token in self.vocab_dict:
                tokens_ret.append(token)
                # else:
                #     tokens_ret.append('[UNK]')  # If not found in vocab, mark as unknown
