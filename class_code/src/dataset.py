...
def create_residual_dataset(batch_size=1, data_file_path=None):
    """create finetune or evaluation dataset"""

    type_cast_int = C.TypeCast(mstype.int32)
    type_cast_float = C.TypeCast(mstype.float32)
    type_cast_float_16 = C.TypeCast(mstype.float16)
    data_set = ds.MindDataset([data_file_path],
                              columns_list=["input_ids", "input_mask", "segment_ids","all_pssms","all_dssps","pssms","dssps","window_seq","label_ids"],
                              shuffle=True)

    data_set = data_set.map(operations=type_cast_int, input_columns="label_ids")
    data_set = data_set.map(operations=type_cast_int, input_columns="segment_ids")
    data_set = data_set.map(operations=type_cast_int, input_columns="input_mask")
    data_set = data_set.map(operations=type_cast_int, input_columns="input_ids")
    data_set = data_set.map(operations=type_cast_float, input_columns="pssms")
    data_set = data_set.map(operations=type_cast_int, input_columns="dssps")
    data_set = data_set.map(operations=type_cast_float_16, input_columns="all_pssms")
    data_set = data_set.map(operations=type_cast_int, input_columns="all_dssps")
    data_set = data_set.map(operations=type_cast_int, input_columns="window_seq")
...
def create_concat_dataset(batch_size=1, assessment_method="accuracy", data_file_path=None,
                                  schema_file_path=None, dataset_format="mindrecord", do_shuffle=True):
    """create finetune or evaluation dataset"""

    type_cast_op = C.TypeCast(mstype.int32)
    data_set = ds.MindDataset([data_file_path],
                              columns_list=["input_ids_0", "input_mask", "segment_ids","input_ids_1","label_ids"],
                              shuffle=do_shuffle)
    ...
    data_set = data_set.map(operations=type_cast_op, input_columns="segment_ids")
    data_set = data_set.map(operations=type_cast_op, input_columns="input_mask")
    data_set = data_set.map(operations=type_cast_op, input_columns="input_ids_0")
    data_set = data_set.map(operations=type_cast_op, input_columns="input_ids_1")
...
def create_classification_independent_dataset(batch_size=1, assessment_method="accuracy", data_file=None,
                                  schema_file_path=None, dataset_format="mindrecord", do_shuffle=True):
    """create finetune or evaluation dataset"""
    type_cast_op = C.TypeCast(mstype.int32)
    if dataset_format == "mindrecord":
