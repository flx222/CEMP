...
class FullTokenizer():
    ...
    def tokenize(self, text, token_type=None):
        """
        Do full tokenization.
        Args:
            text: str of text.
            token_type: str indicating whether the text is 'seq' or 'smile'.
        ...
        elif token_type == 'smile':
            tokens = self._tokenize_smile(text)
            for token in tokens:
                if token not in vocab_list:
                    tokens_ret.append('[UNK]')
                else:
    ...
    def _tokenize_smile(self, text):
        """Tokenizes a SMILES string."""
        tokens = []
        i = 0
        while i < len(text):
            if text[i].isalnum():
                tokens.append(f'SM_{text[i]}')
                i += 1
            else:
                tokens.append(f'SM_{text[i]}')
                i += 1
