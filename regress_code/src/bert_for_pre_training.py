...
class BertPreTrainingWithoutNSP(nn.Cell):
    """
    Bert pretraining network.
    ...
    def __init__(self, config, is_training, use_one_hot_embeddings):
        super(BertPreTrainingWithoutNSP, self).__init__()
        self.bert = BertModel(config, is_training, use_one_hot_embeddings)
        self.cls1 = GetMaskedLMOutput(config)
...
class BertPretrainingLossWithoutNSP(nn.Cell):
    """
    Provide bert pre-training loss.
    ...
    def __init__(self, config):
        super(BertPretrainingLossWithoutNSP, self).__init__()
        self.vocab_size = config.vocab_size
        self.onehot = P.OneHot()
        self.on_value = Tensor(1.0, mstype.float32)
        self.off_value = Tensor(0.0, mstype.float32)
...
class BertNetworkWithLossWithoutNSP(nn.Cell):
    """
    Provide bert pre-training loss through network.
    ...
    def __init__(self, config, is_training, use_one_hot_embeddings=False):
        super(BertNetworkWithLossWithoutNSP, self).__init__()
        self.bert = BertPreTrainingWithoutNSP(config, is_training, use_one_hot_embeddings)
        self.loss = BertPretrainingLossWithoutNSP(config)
        self.cast = P.Cast()
    ...
    def construct(self,
                  ...
                  masked_lm_weights):
        """Get pre-training loss"""
        prediction_scores = self.bert(input_ids, input_mask, token_type_id, masked_lm_positions)
        total_loss = self.loss(prediction_scores, masked_lm_ids, masked_lm_weights)
        return self.cast(total_loss, mstype.float32)
