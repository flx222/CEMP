...
def parse_args():
    ...
    parser.add_argument("--data_dir", type=str, default="./example/fine-tune/dataset/fold_0")
    ...
    parser.add_argument("--output_dir", type=str, default="./example/fine-tune/dataset/fold_0",
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    tokens_a = tokenizer.tokenize(example.text_a, token_type='seq')
    tokens_b = tokenizer.tokenize(example.text_b, token_type='smile')

    print(f"tokens_a: {tokens_a}")  # 调试信息
    print(f"tokens_b: {tokens_b}")  # 调试信息
...
class TnewsProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for i in range(len(lines)):
            guid = "%s-%s" % (set_type, i)
            text_a = lines['seq'][i]
            text_b = lines['smile'][i]
            label = float(lines['label'][i])
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label, data_format=2))
        return examples
